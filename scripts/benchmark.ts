...
import {generateTerrain,spawnPoint} from '../src/sim/terrain';
...
for(const scenario of ['generated','autonomous'] as const){
  ...
  const generationMs=performance.now()-generation,times:number[]=[],samples:number[]=[],observationTimes:number[]=[],e=new Exploration(w),player=spawnPoint(w);
  for(let i=0;i<5400;i++){const start=performance.now();f.step();w.step();const observation=performance.now();e.update(player.x,player.y);observationTimes.push(performance.now()-observation);times.push(performance.now()-start);if(i%900===899)samples.push(f.gold);}
  const typedArrayBytes=Object.values(w).filter(v=>ArrayBuffer.isView(v)).reduce((s,v)=>s+(v as Uint8Array).byteLength,0);
  report.scenarios.push({scenario,dimensions:[w.width,w.height],ticks:5400,simulatedSeconds:180,generationMs,meanMs:times.reduce((a,b)=>a+b,0)/times.length,p95Ms:percentile(times,.95),p99Ms:percentile(times,.99),maxMs:Math.max(...times),typedArrayMiB:typedArrayBytes/1024**2,cartographyMiB:(e.discoveredCells.byteLength+e.rememberedMaterial.byteLength)/1024**2,observationMeanMs:observationTimes.reduce((a,b)=>a+b,0)/observationTimes.length,goldEvery30Seconds:samples,counters:f.counters,remainingWater:w.count(Mat.Water),processMemory:process.memoryUsage()});
}
